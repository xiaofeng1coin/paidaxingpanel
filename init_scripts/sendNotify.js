...
// =========================================================================
// 【核心修改区】: 剥离对青龙内部接口的强依赖，解决 5600 端口拒绝连接报错
// =========================================================================
// 原代码：const { getEnvs } = require('./ql');
// 现代码：伪造一个返回空环境的函数，跳过青龙专属的京东别名处理逻辑
const getEnvs = async () => { return []; }; 
const fs = require('fs');
// 原代码：let strCKFile = '/ql/scripts/CKName_cache.json';
// 现代码：修改为当前相对路径，避免因为没有青龙文件夹结构而报错
...
let strUidFile = './WxPusherUid.json';
...
async function sendNotify(text, desp, params = {}, author = '\n🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉🎉\n通知：脚本执行完毕\n口号：羊毛万岁') {
    ...
    try {
        ...
        if (ShowRemarkType != "1" && titleIndex3 == -1) {
            ...
            // [兼容修改] 绕过请求 127.0.0.1:5600 导致的报错
